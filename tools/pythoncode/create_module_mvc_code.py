...
members=[u'张志强',u'陈斌杰',u'蒙磊',u'蔡世达',u'汤凯']
class ModuleMvc:
	def input_class_name(self):
		while(1):
			print (u'请输入类名根名字,类名一定要驼峰命名,比如登陆界面,类名就是Login');
			print (u'如果是登陆界面,生成的模块文件夹就是Login,mvc代码文件就是Login');
			print (u'按n键退出');
			a = raw_input();
			if a=='n':
				exit();
	...
	def check_class_exist(self): 
		...
		self.m_ctrl_cls_name=self.m_class_name[0].upper()+self.m_class_name[1:]+'Ctrl'; 
