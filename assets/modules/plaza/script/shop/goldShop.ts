...
//c, 控制
@ccclass
export default class GoldShopCtrl extends BaseCtrl {
	protected view: View;
	protected model: Model;
	//这边去声明ui组件
	@property(cc.Node)
	...
	//按钮或任何控件操作的回调begin
	buy() {
		ModuleMgr.getInstance().showMsgBox({ content: `确定要花费${this.cost}购买：${this.name},获得${this.gold+this.gift}?` })
	}
	//end

	// update(dt) {}
	setData(cost: number, gift: number, gold: number, name: string) {
		this.cost = cost;
		this.gift = gift;
		this.gold = gold;
		this.name = name;
